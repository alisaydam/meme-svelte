<script>
  ...
  const changeTheme = () => {
    ...
    themeToggle = themeToggle === "dark" ? "light" : "dark";
    if (mode === "dark") {
...
<a on:click={changeTheme}>
  <svg class="themeToggle" style="width:30px;height:30px" viewBox="0 0 24 24">
    <path
      fill={themeToggle !== "dark" ? "currentColor" : "#F9D71C"}
      d={themeToggle !== "dark" ? moonPath : sunPath}
    />
  </svg>
</a>
